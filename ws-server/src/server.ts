...
import { AuthenticatedWebSocket, IncomingWSMessage, ServerStats } from './types';
import { logger } from './utils/logger';
import { initRedis, checkRedisHealth, pollChannel } from './utils/redis';
 ...
 */
async function subscribeToChannel(chatId: string): Promise<void> {
  ...
  const cleanup = await pollChannel(`chat:${chatId}`, (message) => {
...
wss.on('connection', async (ws: WebSocket, req: IncomingMessage) => {
  ...
  try {
    ...
    authWs.on('close', (code, reason) => {
      ...
      const stats = getConnectionStats();
      const chatStillActive = stats.chatRooms.some(
        room => room.chatId.startsWith(authWs.chatId.substring(0, 8))
      );
