...
export function broadcastToRoom(
  ...
  room.forEach((client) => {
    if (client !== excludeWs && client.readyState === WebSocket.OPEN) {
      try {
        ...
        logger.error(
          `Failed to send message to client in chat ${chatId}`,
          error
        );
      }
    }
  ...
  logger.debug(
    `Broadcasted message to ${sentCount} clients in chat ${chatId}`,
    {
...
export function getConnectionStats(): {
  ...
  const chatRoomStats = Array.from(chatRooms.entries()).map(
    ([chatId, clients]) => ({
  ...
  const totalConnections = Array.from(chatRooms.values()).reduce(
...
export function checkHeartbeats(wss: any): void {
  wss.clients.forEach((ws: AuthenticatedWebSocket) => {
    if (!ws.isAlive) {
      logger.warn(
        `Terminating dead connection for user ${ws.userId} in chat ${ws.chatId}`
      );
      return ws.terminate();
    }
