import { AuthenticatedWebSocket, IncomingWSMessage, SavedMessage } from '../types';
...
export async function handleChatMessage(
  ...
  try {
    logger.debug(`Processing chat message from user ${userId} in chat ${chatId}`);
    
    // Save message to database via Next.js API
    const response = await fetch(`${NEXT_API_URL}/api/chats/${chatId}/messages/send`, {
      method: 'POST',
      headers: {
    ...
    logger.error(`Failed to handle chat message for user ${userId} in chat ${chatId}`, error);
...
export async function handleAgentJoined(
  ...
  try {
    await publishToChannel(`chat:${chatId}`, payload);
    logger.info(`Agent joined broadcast sent`, { chatId, agentId, agentName });
  } catch (error) {
    logger.error('Failed to publish agent_joined to Redis', error);
  }
}
